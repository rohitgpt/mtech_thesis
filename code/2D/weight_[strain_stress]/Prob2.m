function [M, min_x, max_x, x] = Prob2(nx, ny, E, rho, p, max_strain, max_stress, force)
m = length(E);
[U] = @(X) finite_element(nx, ny, E, m, X, p, force);
...
%     M{(ely-1)*2*nx+2*elx+1} = @(X) max(e(X)*C*B*Ue(X)/(1-nu^2)) - max_stress;

%     length(subindex(Ue(X), 1, ':'))-1;
%     subindex(
  end
end
...
%%%%%%%%%% FE-ANALYSIS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [U]=finite_element(nx,ny, E, m, x, p, force)
[KE] = lk; 
