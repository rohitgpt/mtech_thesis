function [M, min_x, max_x, x] = Prob1(nx, ny, E, rho, p, max_stress)
m = length(E);
...
%calculate stress using U and shape function
epsilon = 0.001;
min_x = epsilon*ones(ny,m*nx);
max_x = (1-10*epsilon)*ones(ny,m*nx);
...
% end
s = dot(xe, rho)*(sum(xe))^(p-1)*Ue'*KE*Ue;
...
F([1+2*(ny+1)*nx:2:2*(ny+1)*(nx+1)],1) = 1e5;
...
% B = sym('b', [3 8]);
% for i=1:4
%   B(1:3,i*2-1:i*2) = [diff(n(i), x) 0; 0 diff(n(i), y); diff(n(i), y) diff(n(i),x)];
% end
% syms E nu;
% e = E/(1-nu^2)*[1 nu 0; nu 1 0; 0 0 (1-nu)/2];
% k = B'*e*B;
% int(int(k, x, [-1, 1]), y, [-1 1]) 