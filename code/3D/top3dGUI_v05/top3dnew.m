...
function top3dnew(nelx,nely,nelz,volfrac,penal,rmin)
...
[il,jl,kl] = meshgrid(nelx, 0:nely, 0:nelz);                 % Coordinates
loadnid = kl*(nelx+1)*(nely+1)+il*(nely+1)+(nely+1-jl); % Node IDs
loaddof = 3*loadnid(:);                             % DOFs
% USER-DEFINED SUPPORT FIXED DOFs
[iif,jf,kf] = meshgrid(0,0:nely,0:nelz);                  % Coordinates
fixeddof = fd(iif, jf, kf, nelx, nely, nelz,0)
% fixeddof = union(fixeddof, fd(iif, jf, kf, nelx, nely, nelz, 1));
% fixeddof = union(fixeddof, fd(iif, jf, kf, nelx, nely, nelz, 2));
[iif,jf,kf] = meshgrid(0:nelx,0,0:nelz);                  % Coordinates
fixeddof = union(fixeddof, fd(iif, jf, kf, nelx, nely, nelz, 1));
[iif,jf,kf] = meshgrid(0:nelx,nely,0:nelz);                  % Coordinates
fixeddof = union(fixeddof, fd(iif, jf, kf, nelx, nely, nelz, 1));
[iif,jf,kf] = meshgrid(0:nelx,0:nely,0);                  % Coordinates
% fixeddof = union(fixeddof, fd(iif, jf, kf, nelx, nely, nelz, 1));
% fixeddof = union(fixeddof, fd(iif, jf, kf, nelx, nely, nelz, 0));
fixeddof = union(fixeddof, fd(iif, jf, kf, nelx, nely, nelz, 2));
[iif,jf,kf] = meshgrid(0:nelx,0:nely,nelz);                  % Coordinates
fixeddof = union(fixeddof, fd(iif, jf, kf, nelx, nely, nelz, 2));
% fixeddof = union(fixeddof, fd(iif, jf, kf, nelx, nely, nelz, 1));
% fixeddof = union(fixeddof, fd(iif, jf, kf, nelx, nely, nelz, 0));
...
function [fixeddof] = fd(iif, jf, kf, nelx, nely, nelz, alpha)
fixednid = kf*(nelx+1)*(nely+1)+iif*(nely+1)+(nely+1-jf); % Node IDs
fixeddof = [3*fixednid(:)-alpha];%; 3*fixednid(:)-1; 3*fixednid(:)-2]; % DOFs
