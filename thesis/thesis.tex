...
It is assumed that the wind load on the bamboo stem is uniform, as it is similar to fluid flow over a surface which achieves uniform flow at a height much less than the typical height of the bamboo.Therefore, the bending moment as a function of height will 
\begin{equation}
\begin{split}
M &= Fx = qx\,dx &\qquad (q \text{ is the uniform loading})\\
&= \int_{x}^Lqx\,dx = \frac{qx^2}{2}\bigg|^L_x = \frac{qL^2}{2}(1-\frac{x^2}{L^2})
\end{split}
\end{equation}
Similarly, radius of curvature $R$ follows from
\begin{equation}
R = \frac{EI}{M}
\end{equation}
\begin{figure}[H]
